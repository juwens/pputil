...
* interactive TUI mode with navigable table and detailed profile view

Note:

Single aspects of mobileprovision files can be extracted in relativ cumbersome manual steps with the cli-tools plutil, security and certtool. +
Calling these tool in cascade for (potentially) hundreds of files would be not feasible.

== Links

* fundamentals of provisioning profiles structure and extraction: https://developer.apple.com/documentation/technotes/tn3125-inside-code-signing-provisioning-profiles
...
=== list (default)
Display a compact table with one profile per line. Ideal for a quick overview of all profiles.

```
user@mymac % pputil list
```

=== tui
Interactive TUI mode with navigable table and detailed profile view.

* Use ↑/↓ arrow keys or j/k to navigate through profiles
* Press Enter to view detailed information about the selected profile
* Automatically adapts to your terminal's color scheme using environment detection
* Press q or Esc to quit

```
user@mymac % pputil tui
```

=== list-ext
Display an extended table with several lines per profile. When you need more detailed information.

```
user@mymac % pputil list-ext
```
